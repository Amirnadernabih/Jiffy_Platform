...
export default function CustomerSignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Pass 'user' as required role for customer sign-in
      // eslint-disable-next-line no-unused-vars
      const userData = await login(email, password, 'user');
...
<div className="Get-started">
        ...
        <Col xs={12} md={6} className="d-flex align-items-center justify-content-center py-5 py-md-0 right-side">
          <div className="auth-right-panel w-100" style={{ maxWidth: 420 }}>
            <h3 className="fw-bold mb-1 text-center">Sign In</h3>
            <p className="text-muted small text-center mb-4">Welcome back! Please enter your details.</p>
            ...
            <p className="small text-center mt-4">
              Don't have an account? <a href="/auth/customer/sign-up">Sign up</a>
            </p>

            {/* Demo credentials for testing */}
            <div className="mt-4 p-3 bg-light rounded">
              <small className="text-muted d-block mb-2">Customer Demo Credentials:</small>
              <small className="text-muted d-block">User Only: Mai@gmail.com / Mai123@</small>
            </div>
          </div>
        </Col>
      </Row>
