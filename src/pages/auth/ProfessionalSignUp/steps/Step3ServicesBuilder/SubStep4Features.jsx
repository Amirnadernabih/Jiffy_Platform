...
// Import feature template images (provided paths)
import textOnlyImage from "../../../../../assets/text_only.png";
import imageEachFeature from "../../../../../assets/image_for_each_feature.png";
import centralImage from "../../../../../assets/central_image.jpg";

export default function SubStep4Features({ data, onUpdate, onNext, onPrev }) {
  ...
  useEffect(() => {
    if (data) {
      setSelectedTemplate(data.featuresSelectedTemplate || "");
      setFeatureTitle(data.featureTitle || "");
      setFeatureDescription(data.featureDescription || "");
      setImageFile(data.featureImage || null);
      // Restore attachments previews if available (best-effort; files not serializable across reloads)
      if (Array.isArray(data.featureAttachments) && data.featureAttachments.length > 0) {
        const restored = data.featureAttachments.map((file) => ({ file, url: URL.createObjectURL(file) }));
        setAttachments(restored);
      }
    }
  }, [data]);

  const commitUpdate = (payload = {}) => {
    onUpdate({
      featuresSelectedTemplate: selectedTemplate,
      featureTitle,
      featureDescription,
      featureImage: imageFile,
      featureAttachments: attachments.map((a) => a.file),
      ...payload,
    });
  };
  ...
  const handleTemplateSelect = (templateId) => {
    ...
    commitUpdate({ featuresSelectedTemplate: templateId });
  };

  const handleTitleChange = (value) => {
    setFeatureTitle(value);
    commitUpdate({ featureTitle: value });
  };

  const handleDescriptionChange = (value) => {
    setFeatureDescription(value);
    commitUpdate({ featureDescription: value });
  };
  ...
  const addFiles = (fileList) => {
    ...
    if (valid.length > 0) {
      setAttachments((prev) => {
        ...
        setImageFile(hero);
        commitUpdate({ featureImage: hero, featureAttachments: next.map((a) => a.file) });
        return next;
      });
    }
  };
  ...
  const canContinue = () => {
    return selectedTemplate && featureTitle.trim().length > 0 && featureDescription.trim().length > 0;
  };
  ...
  // Only three templates as requested
  const templates = [
    {
      id: "central-image",
      name: "Central Image",
      description: "Highlight a single central image as hero focus.",
      preview: centralImage,
    },
    {
      id: "text-only",
      name: "Text Only",
      description: "Minimal layout with title and description only.",
      preview: textOnlyImage,
    },
    {
      id: "image-per-feature",
      name: "Image for Each Feature",
      description: "Show a distinct image alongside each feature item.",
      preview: imageEachFeature,
    },
  ];
    ...
    <>
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <div className="d-flex align-items-center gap-3">
          ...
          <span className="step-counter">Step 4/5</span>
        </div>
      </div>

      {/* Constrained container to match SubStep1 layout */}
      <div className="hero-sections-container">
        {/* Template Selection Grid: three cards */}
        <div className="templates-grid mb-4">
          {templates.map((template) => (
            ...
            >
              <div className="template-preview">
                <img src={template.preview} alt={template.name} className="template-image" />
              </div>
        ...
        {/* Inputs and Upload section retains exact class structure */}
        <div className="prototype-form-section inputs-upload-section">
          <div className="inputs-column">
            <div className="prototype-form-group">
              <label>Feature Title<span className="required">*</span></label>
              <div className="prototype-input-wrapper">
                ...
                />
                <span className="character-counter">{featureTitle.length}/60</span>
              </div>
            </div>

            <div className="prototype-form-group">
              <label>Feature Description<span className="required">*</span></label>
              <div className="prototype-input-wrapper">
                <textarea
                  placeholder="type here..."
                ...
                />
                <span className="character-counter">{featureDescription.length}/200</span>
              </div>
            </div>
          </div>
      ...
      <div className="form-navigation">
        ...
        <Button type="button" className="btn-continue" disabled={!canContinue()} onClick={onNext}>
          Continue
        </Button>
      </div>
    </>
